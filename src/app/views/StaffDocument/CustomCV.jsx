...
const CustomCV = (props) => {
    ...
    const handleShowExperienceDialog = () => {
                ...
                <div className='left-content'>
                    <div className="cv-profile">
                        ...
                        <p className="profile-phone"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            ...
                            <span>{formData?.phone}</span></p>
                    </div>
                    <div className="cv-skills">
                        <h4 className="skills-tittle">kỹ năng
                            <IconButton size="small" onClick={() => {handleShowFormSkill()}}>
                                <Icon fontSize="small" color="primary">
                                    edit
                                </Icon>
                            </IconButton></h4>
                        <div>
                            {isEditSkill && (<><TextValidator
                                multiline
                                rows={skillLine}
                                fullWidth
                                variant="outlined"
                        ...
                        <ul className="skill-list">
                            {!isEditSkill &&  formData?.skill && splitStringByNewLine(formData?.skill) !== null && splitStringByNewLine(formData?.skill).map((formData, index) => <li key={index}>{formData}</li>)}
                        </ul>
                    </div>
                    <div className="cv-skill-rating">
                        <h4 className="skill-rating-tittle">Ngoại ngữ</h4>
                        <div className="skill-rating-content">
                            <div className="skill-rating-content-formData">
                                <span>Tiếng anh</span>
                    ...
                    <div className="cv-activity">
                        <h4 className="activity-tittle">Hoạt động
                            <IconButton size="small" onClick={() => {handleShowFormActivity()}}>
                                <Icon fontSize="small" color="primary">
                                    edit
                                </Icon>
                            </IconButton>
                        ...
                        <div className="activity-content">
                            <div className="activity-formData">
                                ...
                                errorMessages={[t('staff.notify.errorMessages_required')]}
                            />
                                <Button className="mt-12 mr-12" variant="contained" color="primary" onClick={handleSubmitActivity}>
                                    {t("general.save")}
                                </Button>
                                <Button className="mt-12 color-error" variant="contained" onClick={handleCancel}>
                                    {t("general.cancel")}
                                ...
                                <ul className="activity-list">
                                    {!isEditActivity && formData?.activity && splitStringByNewLine(formData?.activity) !== null && splitStringByNewLine(formData?.activity).map((formData, index) => <li key={index}>{formData}</li>)}
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <div className='right-content'>
                    ...
                    <div className="cv-experiences border-left">
                        <h3 className="experiences-heading">
                            ...
                            <Fab color="primary" aria-label="Add" className="icon-button-md" onClick={() => {handleShowExperienceDialog()}}>
                                <Icon>add</Icon>
                            </Fab>
                        </h3>
                        <div className="cv-experience">
                            <h4 className="experience-tittle">
                                ...
                                <div>
                                    <IconButton size="small" onClick={() => {handleShowExperienceDialog()}}>
                                        <Icon fontSize="small" color="primary">
                                            edit
                                        </Icon>
                                    </IconButton><IconButton size="small" onClick={() => { }}>
                    ...
                    <div className="cv-certificates border-left">
                        ...
                        <div className="certificates-list">
                            {formData?.certificatesDto !== null && formData?.certificatesDto?.map(formData => <span className="certificates-detail"><span className="detail-dot">&#x2022;</span><span className="detail-content">{moment(formData?.issueDate)?.year()}:{formData?.content}.</span></span>)}
                        </div>
                    </div>
                </div>
                { showExperienceDialog && <ExperienceDialog t={t} handleCloseDialog ={handleCancel}/>}
            </div>
